...
I want to develop a cross-platform (Windows, Linux, and macOS) build system and project and package manager with Batch/Shell and CMake.

You will be the Lead Project Manager and will be responsible for solving the tasks that I'll assign to you.

The idea is that we will have a root directory known as the `Workspace`, which will contain other subdirectories known as `Projects`. Every `Workspace` should contain at least one main `Project` that will contain all the required Batch and CMake files.

`Projects` are composed of `Targets` (not related to CMake target) and `Modules` (not related to C++20 module) that reside within their `Source` subdirectories. Each `Project` must have a metadata file (`<ProjectName>.Project.cmake`) that describes it with information such as: `Name`, `Version`, `Author(s)`, `Category`, `Targets`, `Platforms\Architectures`, etc.
...
`Targets` and `Modules` should be stored in the `Source` directory of a `Plugin` or `Project`. The root folder of each `Module` should match its name but it is not strict rule.
...
At the top level of the directory structure, you will find the **Main Project** directory alongside any individual project directories. The **Main Project** directory contains the core system and its associated tools, while each project directory holds all files specific to that project.
...
└── Setup.sh
```
- `GenerateProjectFiles` - Used to generate the project files needed to work with the base and its corresponding game(s) within your preferred IDE (such as Visual Studio, Visual Studio Code, Xcode, and others).
...
- Do not make emotional comments;

After I've finished sending this entire prompt you should only:
- Ask about the current progress;
- Ask about your current task;
  
After I've sent the current progress and current task, you should just respond the necessary information regarding the task and nothing more, just wait for me to analyze and test.
